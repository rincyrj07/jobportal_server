...
const createCompanyController = async (req, res) => {
    ...
    // try {
        ...
        const company = await (await CompanyModel.create(data)).populate("employer")
...
const companyListController = async (req, res) => {
    ...
    // try {
    const user = req.user
    const companyList = await CompanyModel.find({ employer: user._id })
    res.json({ "message": "Gym list fetched successfully", companyList })
    // } catch (err) {
    //         res.status(500).json({ message: "Something went wrong in the server. Please try after some time" })
    // }
